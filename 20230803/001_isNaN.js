...
// isNaN() 함수는 어떤값이 숫자가 아닌지 판별합니다
// 숫자로 판별되지 않는 것들은 false로 판별
// 숫자로 판별되 않는 것들은 true를 반환합니다.


// IEEE 국제 표준에 정의된 것을 의미(isNaN, 인피니티)
...
// "p2o4i8gj2"
// 쉽게 푸는 방법
// 암기 : isNaN은 Number로 숫자 변환이 가능한 값을 flase로 준다
// !isNaN(value[0]) 숫자를 걸러내는 방법

let value = "p2o4i8hj2"

isNaN(value[0]) // p, ture
isNaN(value[1]) // 2, false
isNaN(value[2]) // o, true
// ...생략....

// 문제는 다 더하는 것으로 고침. 정렬을 우리가 안 배웠기 때문
let value = "p2o4i8gj2"
let sum = 0

// !isNaN 실행을 방지하기 위해 사용
if(!isNaN(value[0])){
...
if(!isNaN(value[4])){
    sum += parseInt(value[4])
}
if(!isNaN(value[5])){
    sum += parseInt(value[5])
}
if(!isNaN(value[6])){
    sum += parseInt(value[6])
}
if(!isNaN(value[7])){
    sum += parseInt(value[7])
}
console.log(sum) // 14 (숫자들 끼리의 합)
...
// 8. numberisNaN (NaN만 true 반환)과 isNaN의 차이
