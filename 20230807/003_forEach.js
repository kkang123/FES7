...
// 문제2. forEach를 사용해서 짝수 `인덱스`의 값을 모두 더하세요. (0도 짝수 인덱스)
...
// Array(100).fill(0)와 같은 코드 대신 뒤에서 배울 map을 더 많이 사용한다.
// 문제3. 1부터 100까지의 숫자를 다 더해주세요.
Array(100).fill(0).forEach((v,i) => {
    //console.log(i) //0부터 99까지
    console.log(i+1) //1부터 100까지
})
...
// 한글풀이
// let userInput = [3, 76 , 24]라면 정렬된 후의 값은 [76, 24, 3]이 됩니다. 그렇다면 정렬된 후의 값에 3이 위치한 인덱스는 2가 됩니다. 76이 위치한 인덱스는 0이 됩니다. 24가 위치한 인덱스는 1이 됩니다.
// 그렇다면 userInput에 이 인덱스를 하나씩 지정합니다. 그러면 userInput은 [2, 0, 1]이 됩니다.
// 결과값을 보니 0 번째는 없기 때문에 여기에 + 1 해준다. [3, 1, 2]가 되는 것이다.
